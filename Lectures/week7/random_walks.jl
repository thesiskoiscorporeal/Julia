...
# ╔═╡ ab99d63e-199b-44ca-a49b-231aad24d2e3
# ENV["GRDIR"] = ""

# ╔═╡ 50cab586-8b03-49a8-b67e-7c7158aa46fb
# Pkg.build("GR")

# ╔═╡ 5090d7c8-2b31-45d4-827f-f5242aab7abc
# Pkg.add("PlotlyJS")

# ╔═╡ f0d84010-a811-47ce-bdf3-5b85c4b5997b
# Pkg.add([
# 	Pkg.PackageSpec(name="GR", version="0.57.2"),
# ])

# ╔═╡ b3faa86f-a6ad-49dc-82c2-41ece17901db
# using GR

# ╔═╡ 059aba3a-e239-4d9c-8a43-4c0294fb526c
with_terminal() do
	Pkg.status("Plots")
end
...
begin
	...
	for i in 1:250
		plot!(walk1D(100), leg=false, lw=2, alpha=0.5)
	end
...
# ╔═╡ 4603e92f-4938-4a83-92f1-da5f9f8fa68f
md"""
## Why not try a walker in 3D?
"""

# ╔═╡ e600e9ea-f8c4-4410-b7d3-647a1dddcae9
struct Walker3D <: Walker
	x::Int
	y::Int
	z::Int
end

# ╔═╡ 53a64796-0df2-466f-bf3e-807a7c561fb4
position(w::Walker3D) = (w.x,w.y,w.z)

# ╔═╡ 3c3971e2-91da-11eb-384c-01c627318bdc
update(w::W, step) where {W <: Walker} = W(position(w) + step) #generic programming, now, if need be, I can define position() and step() for every subtype of Walker I create. No need for a new update() function each time.

# ╔═╡ 3925b772-fcc6-4f49-8b22-b546aa775e09
step(w::Walker3D) = rand( [ [i,j,k] for i = [-1,1], j=[-1,1], k=[-1,1] ] )

# ╔═╡ 0b356233-2403-4879-9384-12f03f41fcd8
update(w::Walker3D, step::Vector) = Walker3D(w.x + step[1], w.y + step[2], w.z + step[3])
...
trajectory(Walker3D(0,0,0), 3)

# ╔═╡ 5e6c2d45-f215-42ce-a987-0244fac8ee5a
Traj = trajectory(Walker3D(0,0,0), 10^N)
