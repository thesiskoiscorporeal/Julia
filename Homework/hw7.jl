...
begin
    ...
    using Plots, PlutoUI, StatsBase
end

# ╔═╡ f71322c1-8f16-4517-b173-9a1e0378a56c
using Statistics
...
function generate_agents(N::Integer)
	output = [Agent() for i in 1:N]
	output[rand(1:N)].status = I
	return output
end

# ╔═╡ 61aae66c-a365-4477-8ecb-d4626b73dd8d
generate_agents(5)
...
md"""
##### note that the mean curve tends to be lower than the ones you see, to account for all the infection curves that remain at 0
"""
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	plot(1:T, s_means, lw=3, c=:blue, leg=false)
	plot!(1:T, i_means, lw=3, c=:red)
	plot!(1:T, r_means, lw=3, c=:green)
...
# ╔═╡ 156522b1-9d5d-4970-804c-795d3026e44b
md"""
##### it's sigmoidal!
"""
...
2. % of populaton infected 
3. time taken until no more infected
"""
...
function interact!(agent::Agent, source::Agent, infection::Reinfection)
	if agent.status == I
		if rand()<infection.p_recovery
			agent.status = S #agent becomes susceptible again
		end
	elseif agent.status == S
		if source.status == I
			if rand()<infection.p_infection
...
let
	agent = Agent(S, 0)
	source = Agent(I, 0)
	infection = InfectionRecovery(0.7, 0.5)
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	a_idx, s_idx = sample(1:length(agents), 2, replace=false)
	interact!(agents[a_idx], agents[s_idx], infection)
	return agents
end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	for i in 1:T
		...
		append!(s_counts, count(x -> x.status == S, agents))
		append!(i_counts, count(x -> x.status == I, agents))
		append!(r_counts, count(x -> x.status == R, agents))
	end
...
simulation(100, 1000, InfectionRecovery(0.02, 0.002))
...
simulations = repeat_simulations(200, 1000, InfectionRecovery(p_infection, p_recover), 20)
...
let
	...
	means = []
	for t in 1:length(first(simulations).S) #T
		append!(means, mean([sim.I[t] for sim in simulations]))
	end
	
	plot!(1:1000, means, lw=3, c=:red)
...
let
	...
	results = (S=round.(sum(all_S_counts) ./ length(simulations) ./ 100, digits=4),
...
# ╔═╡ 0329711a-38fa-4cdd-9722-f7f34521c2fe
begin
	i_means = []
	
	for t in 1:length(first(simulations).S)
		append!(i_means, mean([sim.I[t] for sim in simulations]))
	end
	
	cumsum(i_means)
	plot(1:length(first(simulations).S), cumsum(i_means))
end
...
# ╔═╡ 176cdb6e-f9d7-4842-ae70-1d398cf35778
cough = Reinfection(0.1,0.02)

# ╔═╡ 1ac4b33a-0435-11eb-36f8-8f3f81ae7844
reinfectsim = repeat_simulations(100,500,cough,20)

# ╔═╡ cccc1b98-183c-481f-abd6-33e121f77c26
let
	p = plot(leg=false)
	
	for sim in reinfectsim
		plot!(p, 1:500, sim.I, alpha=.5, label=nothing)
	end
	
	means = []
	for t in 1:length(first(reinfectsim).S) #T
		append!(means, mean([sim.I[t] for sim in reinfectsim]))
	end
	
	plot!(1:500, means, lw=3, c=:red)
...
# ╔═╡ 8a91e543-3817-407d-97a6-e417b676e803
md"""
##### doesn't go down again! sigmoid up to a plateau
"""

# ╔═╡ 21c50840-0435-11eb-1307-7138ecde0691
sir_mean_plot(reinfectsim)
