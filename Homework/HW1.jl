...
function demean(xs)
	normed_xs = [i - mean(xs) for i in xs]
	return normed_xs
end
...
# ╔═╡ 3550df90-7be2-11eb-15e4-5bec4ba4d978
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ 35fadb30-7be2-11eb-38f3-91795baf1804
function get_greens(image::AbstractMatrix)
	return get_green.(image)
end

# ╔═╡ 6472c2c0-7be2-11eb-270a-a7c7bb4de7ee
...
function mean_color(image)
	return RGB(mean(get_reds(image)), mean(get_greens(image)), mean(get_blues(image))) 		#one line wonder, baby
end
...
# ╔═╡ a817160e-7be3-11eb-1e71-6f5e74de0bd8
quantize(0.35)
...
function custom_filter(pixel::AbstractRGB)	
	return RGB(pixel.r,clamp(pixel.g + 0.3,0,1), clamp(pixel.b + 0.3,0,1))
end
