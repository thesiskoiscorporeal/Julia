...
function greedy_seam(energies, starting_pixel::Int)
	...
	push!(output, starting_pixel) #push starting_pixel to output vector
	pointer = starting_pixel #initialise column pointer
	
	for i in 1:m-1 #iterate over rows 1 to m-1 of energies array
		lookahead = energies[i+1, clamp(pointer, 2, n)-1:clamp(pointer,1,n-1)+1]
		nextindex = clamp(pointer-2+argmin(lookahead), 1, n) #clamp for edge handling
		push!(output, nextindex) #push nextindex to output
		pointer = nextindex #move pointer to nextindex
	end
...
# ╔═╡ a8c57dd0-8931-11eb-28a0-6fb4aabb0a14
md"""
Not very good, huh?
"""
